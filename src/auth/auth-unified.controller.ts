...
export class AuthUnifiedController {
  constructor(private readonly authUnifiedService: AuthUnifiedService) {}
  ...
  async login(@Body() loginDto: LoginDto) {
    const result = await this.authUnifiedService.login(loginDto);
  ...
  async register(@Body() registerDto: CreateMemberDto) {
    const result = await this.authUnifiedService.register(registerDto);
  ...
  async getProfile(@CurrentUser() user: any) {
    const profile = await this.authUnifiedService.getProfile(user.sub);
  ...
  async getPermissions(@CurrentUser() user: any) {
    const permissions = await this.authUnifiedService.getPermissionsSummary(user.sub);
